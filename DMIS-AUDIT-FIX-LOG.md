...
Migration counter: next free = V144 (V140 = ICS command roles; V141 = area-name backfill; V142 = area-role authority trim; V143 = stakeholder_portal.view + OH trim). ASSIGN SERIALLY HERE before creating any migration.
...
- Status: **FIXED 2026-07-06 — role-level (so it auto-applies to ALL accounts holding the role, all 31 RAS + Reg DC/DED/DAS, no per-user work) and enforced BACKEND-side, not just menu-hiding. (a) V142 revoked authority grants from RAS/Reg DC/DED/DAS: early_warning.create, one_health.manage/disseminate, preparedness.manage, command_post.activate/posture, budget_and_finance.manage (tier-approval + views kept). (b) V143 created stakeholder_portal.view (the partner-facing module had NO permission of its own — it was gated on resource_allocation.view, which every area officer legitimately holds, so it leaked onto their menus), granted to Partners/Super Admin/ICT Admin/EOCC/Director/Asst. Director; also removed one_health.view/acknowledge from the four area roles. (c) access.ts: EW AUTHORING routes (scanner dispatch + 7 agency consoles + bulletin composition) now require early_warning.create; portal routes require stakeholder_portal.view. VERIFIED live: 5 sampled RAS accounts across regions all auto-trimmed on fresh login; backend 403s RAS on scanner/scan, eocc/activate, onehealth/events; RAS→scanner/MoW console = access-denied while the EW hub (receive warnings) still opens; fresh RAS hub = exactly 4 cards (Preparedness/Response/Budget&Finance/Reports, screenshot); regressions: MoW keeps its console, EOCC keeps the scanner + cross-agency dispatch, Partners keeps the portal permission. All reversible in Settings → Roles & Permissions. CAVEAT (honest): already-issued JWTs carry the old permission list until re-login/expiry — active sessions must log out/in to see the trimmed menu; a permissions-version claim / forced re-issue is a follow-up.**
