...
class MultiriskBuilder(BaseBulletinBuilder):
    ...
    def _build_assessment_section(self, day: MultiriskDayForecast):
        ...
        # Three tier labels — pure yellow / orange / red fills + BLACK text
        # (matches official TMA samples and PMO expectation for Multirisk chips).
        tier_data = {tier.alert_level: tier for tier in day.alert_tiers}
        tiers_to_show = [
            (AlertLevel.MAJOR_WARNING, "major_warning", HEX_HIGHLIGHT_MAJOR),
            (AlertLevel.WARNING, "warning", HEX_HIGHLIGHT_WARNING),
            (AlertLevel.ADVISORY, "advisory", HEX_HIGHLIGHT_ADVISORY),
        ]

        for alert_level, label_key, bg_hex in tiers_to_show:
            ...
            try:
                set_table_col_widths(inner_table, [2200, 4200])
            except Exception:
                pass

            # Label cell — solid fill + black words (also shade the run so PDF conversion keeps colour)
            label_cell = inner_table.cell(0, 0)
            set_cell_shading(label_cell, bg_hex)
            set_cell_margins(label_cell, top=40, bottom=40, left=60, right=60)
            p = label_cell.paragraphs[0]
            p.alignment = WD_ALIGN_PARAGRAPH.CENTER
            label_text = t(label_key, self.lang)
            run = _add_run(p, label_text, size=SIZE_LABEL, bold=True, color=COLOR_BLACK)
            shade_run(run, bg_hex)
            ...
            if tier and tier.text:
                _add_run(p, f"  {tier.text}", size=SIZE_BODY_SMALL, color=COLOR_BLACK)
            elif tier is None or not tier.recommendations:
                _add_run(p, f"  {none_text}", size=SIZE_BODY_SMALL, color=COLOR_BLACK)

            # Keep a thin black border on the coloured label so the chip reads clearly in PDF
            set_cell_borders(
                label_cell,
        ...
        # PMO directives / recommendations — render ### WARNING etc. as coloured chips
        # (never print raw "### WARNING" or double bullets like "• - text").
        ...
        if recs:
            ...
            if intro:
                ...
                _add_run(p_intro, intro, size=SIZE_BODY_SMALL, bold=True, color=COLOR_BLACK)

            self._render_coloured_composition(
                right_cell, recs, size=SIZE_BODY_SMALL, indent_cm=0.5, container="cell",
            )
    ...
    def _build_dmd_section(self, day: MultiriskDayForecast):
        ...
        # Impact lines — ### WARNING etc. become coloured chips; single bullet only
        bullets = day.dmd_comment.impact_bullets or []
        if bullets:
            self._render_coloured_composition(
                self.doc, bullets, size=SIZE_BODY, indent_cm=0.5, container="doc",
            )
    ...
    # ── Coloured composition (PMO ### LEVEL sections) ─────────────────────────
    ...
    _LEVEL_FILL = {
        "MAJOR_WARNING": HEX_HIGHLIGHT_MAJOR,
        "WARNING": HEX_HIGHLIGHT_WARNING,
        "ADVISORY": HEX_HIGHLIGHT_ADVISORY,
    }

    @staticmethod
    def _parse_level_header(line: str):
        """Return level key if line is a ### / [ ] colour header, else None."""
        import re
        s = (line or "").strip()
        # strip leading bullets before testing header
        s = re.sub(r"^[\-\*\u2022•]+\s*", "", s)
        m = re.match(
            r"^(?:###?\s*)?(?:\[)?\s*(MAJOR[_\s-]?WARNING|WARNING|ADVISORY|RED|ORANGE|YELLOW)\s*(?:\])?\s*:?\s*$",
            s,
            re.I,
        )
        if not m:
            # also match "### WARNING something" only if almost pure header
            m2 = re.match(
                r"^###?\s*(MAJOR[_\s-]?WARNING|WARNING|ADVISORY)\b\s*$",
                s,
                re.I,
            )
            if not m2:
                return None
            raw = m2.group(1).upper().replace(" ", "_").replace("-", "_")
        else:
            raw = m.group(1).upper().replace(" ", "_").replace("-", "_")
        if raw in ("RED", "MAJOR_WARNING", "MAJORWARNING"):
            return "MAJOR_WARNING"
        if raw in ("ORANGE", "WARNING"):
            return "WARNING"
        if raw in ("YELLOW", "ADVISORY"):
            return "ADVISORY"
        return None

    @staticmethod
    def _clean_bullet_text(line: str) -> str:
        """Strip markdown/bullet prefixes so we never print '• - text' or '• ###'."""
        import re
        s = (line or "").strip()
        # remove repeated bullet/dash prefixes
        while True:
            n = re.sub(r"^[\-\*\u2022•]+\s*", "", s)
            if n == s:
                break
            s = n
        # drop pure markdown header leftovers that slipped through
        s = re.sub(r"^###?\s*", "", s).strip()
        return s

    def _render_coloured_composition(self, host, lines, *, size=SIZE_BODY,
                                     indent_cm=0.5, container="doc"):
        """Render authoring lines: colour headers as filled chips, content as single bullets.

        Authoring input may look like::
        ...
        PDF output: red/orange/yellow chips (black text) + one bullet per content line.
        """
        if not lines:
            return
        ...
        for raw in lines:
            ...
            if not text:
                continue
            level = self._parse_level_header(text)
            if level:
                fill = self._LEVEL_FILL[level]
                label = self._LEVEL_LABEL[level]
                p = add_para()
                p.paragraph_format.space_before = Pt(4)
                p.paragraph_format.space_after = Pt(2)
                run = _add_run(p, f" {label} ", size=size, bold=True, color=COLOR_BLACK)
                shade_run(run, fill)
                continue

            body = self._clean_bullet_text(text)
            if not body:
                continue
            # skip if body is still just a level word
            if self._parse_level_header(body):
                continue
            p = add_para()
            p.paragraph_format.space_before = Pt(1)
            p.paragraph_format.space_after = Pt(1)
            p.paragraph_format.left_indent = Cm(indent_cm)
            _add_run(p, "\u2022 ", size=size, bold=True, color=COLOR_BLACK)
            _add_run(p, body, size=size, color=COLOR_BLACK)
    ...
    def _build_summary_page(self):
        ...
        for summary in self.bulletin.day_summaries:
            ...
            # Three tier labels with district lists — pure yellow/orange/red + black words
            tiers = [
                ("major_warning", HEX_HIGHLIGHT_MAJOR, summary.major_warning_districts),
                ("warning", HEX_HIGHLIGHT_WARNING, summary.warning_districts),
                ("advisory", HEX_HIGHLIGHT_ADVISORY, summary.advisory_districts),
            ]

            for label_key, bg_hex, districts in tiers:
                ...
                if districts:
                    ...
                    _add_run(p, district_text, size=SIZE_BODY_SMALL, color=COLOR_BLACK)
