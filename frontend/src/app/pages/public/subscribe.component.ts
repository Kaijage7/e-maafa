    ...
    <div class="v2-page-content" style="max-width: min(1560px, 94vw); margin: 0 auto; padding: 7rem 1.5rem 4rem;">
      ...
      <div style="display:flex;gap:0.5rem;margin-bottom:1.4rem;">
        <button type="button" (click)="tab.set('sub')" [style.background]="tab() === 'sub' ? '#003366' : 'transparent'" [style.color]="tab() === 'sub' ? '#fff' : '#475569'"
                style="border:1px solid rgba(0,51,102,0.25);border-radius:21px;padding:0 1.2rem;min-height:42px;display:inline-flex;align-items:center;font-size:0.95rem;font-weight:600;cursor:pointer;">{{ L.t('sub_subscribe') }}</button>
        <button type="button" (click)="tab.set('unsub')" [style.background]="tab() === 'unsub' ? '#003366' : 'transparent'" [style.color]="tab() === 'unsub' ? '#fff' : '#475569'"
                style="border:1px solid rgba(0,51,102,0.25);border-radius:21px;padding:0 1.2rem;min-height:42px;display:inline-flex;align-items:center;font-size:0.95rem;font-weight:600;cursor:pointer;">{{ L.t('sub_unsubscribe') }}</button>
      </div>
      ...
      @if (tab() === 'sub') {
        @if (!doneId()) {
          <div style="background:var(--card-bg, #fff);border:1px solid rgba(0,0,0,0.08);border-radius:16px;padding:1.4rem;display:grid;gap:0.9rem;">
            ...
            <div>
              <div style="font-size:0.9rem;font-weight:700;color:var(--text-primary, #2C3E50);margin-bottom:0.4rem;">{{ L.t('sub_how_reach_you') }}</div>
              <div style="display:flex;gap:0.4rem;flex-wrap:wrap;">
                @for (c of channelOpts; track c) {
                  <button type="button" (click)="toggle(channels, c)" [style.background]="channels().includes(c) ? '#003366' : 'transparent'" [style.color]="channels().includes(c) ? '#fff' : '#475569'"
                          style="border:1px solid rgba(0,51,102,0.3);border-radius:18px;padding:0.4rem 0.9rem;font-size:0.9rem;cursor:pointer;">{{ c }}</button>
                }
              </div>
            </div>
            <div>
              <div style="font-size:0.9rem;font-weight:700;color:var(--text-primary, #2C3E50);margin-bottom:0.4rem;">{{ L.t('sub_hazards_of_interest') }}</div>
              <div style="display:flex;gap:0.4rem;flex-wrap:wrap;">
                @for (h of hazardOpts; track h) {
                  <button type="button" (click)="toggle(hazards, h)" [style.background]="hazards().includes(h) ? '#d97706' : 'transparent'" [style.color]="hazards().includes(h) ? '#fff' : '#475569'"
                          style="border:1px solid rgba(217,119,6,0.4);border-radius:18px;padding:0.4rem 0.9rem;font-size:0.9rem;cursor:pointer;">{{ h }}</button>
                }
              </div>
            </div>
            ...
            <label style="display:flex;gap:0.5rem;align-items:flex-start;font-size:0.9rem;color:var(--text-secondary, #475569);">
              <input type="checkbox" [checked]="consent()" (change)="consent.set($any($event.target).checked)" style="margin-top:3px;">
              {{ L.t('sub_consent') }}
            </label>
            @if (error()) { <div style="color:#dc2626;font-size:0.9rem;">{{ error() }}</div> }
        ...
        @if (unsubStep() === 'done') {
          <div style="background:var(--card-bg, #fff);border:1px solid rgba(0,0,0,0.08);border-radius:16px;padding:1.6rem;text-align:center;">
            ...
            <p style="font-size:0.95rem;color:var(--text-secondary, #475569);margin:0;">{{ unsubMsg() }}</p>
          </div>
        } @else {
          <div style="background:var(--card-bg, #fff);border:1px solid rgba(0,0,0,0.08);border-radius:16px;padding:1.4rem;display:grid;gap:0.9rem;">
            @if (unsubStep() === 'request') {
              <p style="font-size:0.95rem;color:var(--text-secondary, #475569);margin:0;">{{ L.t('sub_unsub_request_intro') }}</p>
              ...
              <label style="font-size:0.9rem;font-weight:700;color:var(--text-primary, #2C3E50);">{{ L.t('sub_why_unsubscribing') }} <span style="font-weight:500;color:var(--text-secondary,#94a3b8);">{{ L.t('sub_optional') }}</span></label>
              ...
              @if (unsubMsg()) { <div [style.color]="unsubOk() ? '#059669' : '#dc2626'" style="font-size:0.9rem;">{{ unsubMsg() }}</div> }
              ...
              @if (unsubErr()) { <div style="color:#dc2626;font-size:0.9rem;">{{ unsubErr() }}</div> }
              <div style="display:flex;gap:0.6rem;">
                ...
                <button type="button" (click)="unsubStep.set('request'); unsubErr.set('')" style="background:transparent;border:1px solid rgba(0,0,0,0.15);border-radius:8px;min-height:42px;padding:0 0.9rem;font-size:0.95rem;color:#475569;cursor:pointer;">{{ L.t('sub_back') }}</button>
              </div>
            }
          </div>
        }
