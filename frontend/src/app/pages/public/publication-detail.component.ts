    ...
    <div class="v2-page-content" style="max-width: min(1560px, 94vw); margin: 0 auto; padding: 7rem 1.5rem 4rem;">
      @if (pub(); as p) {
        ...
        <div style="margin-top:1rem;display:flex;align-items:center;gap:12px;">
          ...
          <div>
            <div style="font-size:0.8rem;text-transform:uppercase;letter-spacing:1.5px;color:#94a3b8;">{{ p.documentType }} @if (p.yearOfApproval) { · {{ p.yearOfApproval }} }</div>
        ...
        @if (p.narrativeDescription) {
          <p style="font-size:1.05rem;color:var(--text-secondary, #475569);line-height:1.9;margin-top:1.4rem;text-align:justify;">{{ p.narrativeDescription }}</p>
        }
        @if (p.attachmentPath) {
            ...
            @for (r of related(); track r.id) {
              <div class="col-md-6">
                <a [routerLink]="['/publications', r.documentType, r.id]" style="display:block;text-decoration:none;border:1px solid rgba(0,0,0,0.08);border-radius:12px;background:var(--card-bg, #fff);padding:0.9rem 1rem;">
                  <div style="font-size:1.1rem;font-weight:700;color:var(--text-primary, #2C3E50);">{{ r.documentName }}</div>
                  <div style="font-size:0.9rem;color:#94a3b8;margin-top:2px;">{{ r.yearOfApproval || '' }}</div>
                </a>
              </div>
            }
          </div>
