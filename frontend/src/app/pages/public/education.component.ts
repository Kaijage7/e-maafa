    ...
    <div class="v2-page-content" style="max-width: 1320px; margin: 0 auto; padding: 7rem 2rem 4rem;">
      ...
      @if (item(); as it) {
        ...
        <div style="margin-top:1rem;">
          ...
          <h1 style="font-weight:800;color:var(--text-primary, #2C3E50);margin:0.7rem 0 0.4rem;">{{ it.title }}</h1>
          ...
          @if (it.summary) { <p style="font-size:1.02rem;font-weight:600;color:var(--text-primary, #2C3E50);line-height:1.7;">{{ it.summary }}</p> }
          <div style="font-size:1.05rem;color:var(--text-secondary, #475569);line-height:1.85;white-space:pre-line;">{{ it.fullContent }}</div>
        ...
        @if (threats().length) {
          <h5 style="font-weight:800;color:var(--text-primary, #2C3E50);margin:0 0 0.9rem;">{{ L.lang() === 'sw' ? 'Vitisho vya Kitaifa' : 'National Threats Under Watch' }}</h5>
              ...
              <div class="col-md-6">
                <a [routerLink]="['/threats', th.id]" style="display:flex;align-items:center;gap:12px;border:1px solid rgba(0,0,0,0.08);border-radius:12px;background:var(--card-bg, #fff);padding:0.9rem 1rem;text-decoration:none;">
                  ...
                  <div style="min-width:0;">
                    ...
                    <div style="font-size:0.72rem;color:var(--text-secondary, #64748b);">{{ th.sourceAgency }} — {{ L.lang() === 'sw' ? 'soma athari za nyuma kwa Tanzania' : 'see past impacts to Tanzania' }}</div>
                  </div>
        ...
        <h5 style="font-weight:800;color:var(--text-primary, #2C3E50);margin:0 0 0.9rem;">Guides & Articles</h5>
        <div class="row g-3">
          @for (c of contents(); track c.id) {
            <div class="col-md-6 col-lg-4">
              <a [routerLink]="['/education', c.id]" style="display:flex;flex-direction:column;gap:8px;height:100%;text-decoration:none;border:1px solid rgba(0,0,0,0.08);border-radius:14px;background:var(--card-bg, #fff);padding:1.1rem 1.2rem;">
                ...
                <div style="font-size:1.05rem;font-weight:700;color:var(--text-primary, #2C3E50);line-height:1.35;">{{ c.title }}</div>
                @if (c.summary) { <p style="font-size:0.9rem;color:var(--text-secondary, #64748b);line-height:1.6;margin:0;flex:1;">{{ c.summary.slice(0, 130) }}</p> }
            ...
            <div class="col-12 text-center" style="color:var(--text-secondary, #64748b);padding:3rem;">
              ...
              <p style="margin-top:0.8rem;">No educational content published yet.</p>
            </div>
          }
        </div>
      }
