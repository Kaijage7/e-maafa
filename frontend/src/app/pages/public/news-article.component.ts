...
interface NewsArticle { title: string; slug: string; excerpt: string; body: string; image: string; category: string; publishedAt: string; }
  ...
  template: `
    <div class="v2-page-content" style="max-width: 900px; margin: 0 auto; padding: 7rem 1.5rem 4rem;">
      @if (article(); as a) {
        ...
        <h1 style="font-weight:800;color:var(--text-primary, #2C3E50);line-height:1.25;margin:0.6rem 0;">{{ a.title }}</h1>
        ...
        @if (a.image) { <img [src]="a.image" [alt]="a.title" style="width:100%;border-radius:16px;margin-bottom:1.6rem;max-height:420px;object-fit:cover;"> }
        @if (a.excerpt) { <p style="font-size:1.05rem;font-weight:600;color:var(--text-primary, #2C3E50);line-height:1.7;">{{ a.excerpt }}</p> }
        <div style="font-size:0.95rem;color:var(--text-secondary, #475569);line-height:1.9;white-space:pre-line;">{{ a.body }}</div>
            ...
            @for (r of related(); track r.slug) {
              <div class="col-md-4">
                <a [routerLink]="['/news', r.slug]" style="display:block;text-decoration:none;border:1px solid rgba(0,0,0,0.08);border-radius:14px;overflow:hidden;background:var(--card-bg, #fff);">
                  @if (r.image) { <img [src]="r.image" [alt]="r.title" style="width:100%;height:120px;object-fit:cover;"> }
                  <div style="padding:0.8rem 0.9rem;">
                    <div style="font-size:0.7rem;color:#94a3b8;">{{ r.publishedAt }}</div>
                    <div style="font-size:0.86rem;font-weight:700;color:var(--text-primary, #2C3E50);line-height:1.35;">{{ r.title.slice(0, 70) }}</div>
                  </div>
                </a>
              </div>
            }
        ...
        <div style="text-align:center;padding:5rem 1rem;color:var(--text-secondary, #64748b);">
          ...
          <h4 style="margin-top:1rem;">Article not found</h4>
