        ...
        @if (tabMaterials().length) {
              ...
              <div style="display:grid;gap:0.8rem;">
                @for (m of phaseGuides(ph.key); track m.title) {
                  <div style="border:1px solid rgba(0,0,0,0.08);border-radius:14px;background:var(--card-bg, #fff);padding:1.1rem 1.2rem;" [style.border-left]="'4px solid ' + ph.color">
                    <h6 style="font-weight:700;color:var(--text-primary, #2C3E50);margin:0 0 0.6rem;font-size:0.92rem;">{{ mTitle(m) }}</h6>
          ...
          @if (otherMaterials().length) {
            <div style="display:flex;align-items:center;gap:10px;margin:1.3rem 0 0.7rem;">
              ...
              <h5 style="font-weight:800;margin:0;font-size:0.98rem;color:#6366f1;">{{ L.t('hub_videos_resources') }}</h5>
            ...
            @for (m of otherMaterials(); track m.title) {
              <div style="border:1px solid rgba(0,0,0,0.08);border-radius:16px;background:var(--card-bg, #fff);padding:1.2rem 1.3rem;">
                <div style="display:flex;align-items:center;gap:10px;margin-bottom:0.7rem;">
                  ...
                  <h5 style="font-weight:800;color:var(--text-primary, #2C3E50);margin:0;font-size:1rem;">{{ mTitle(m) }}</h5>
                ...
                @if (m.materialType === 'action_guide') {
                  ...
                  @if (mBody(m)) { <p style="font-size:0.85rem;color:var(--text-secondary, #64748b);">{{ mBody(m) }}</p> }
                  @if (embedUrl(m.videoUrl); as src) {
                    <div style="position:relative;padding-top:56.25%;border-radius:12px;overflow:hidden;">
                      <iframe [src]="src" style="position:absolute;inset:0;width:100%;height:100%;border:0;" allowfullscreen [title]="mTitle(m)"></iframe>
                    </div>
                  } @else {
                    <a [href]="m.videoUrl" target="_blank" rel="noopener" style="color:#60a5fa;font-weight:600;font-size:0.88rem;"><i class="fas fa-play-circle me-1"></i>{{ L.t('hub_watch_video') }}</a>
                  }
                } @else {
                  @if (mBody(m)) { <p style="font-size:0.88rem;color:var(--text-secondary, #475569);line-height:1.7;white-space:pre-line;margin:0;">{{ mBody(m) }}</p> }
                  @if (m.fileUrl) {
          ...
          <div style="text-align:center;padding:2.5rem;border:1px dashed rgba(0,0,0,0.12);border-radius:16px;color:var(--text-secondary, #64748b);">
            ...
            <p style="margin:0.7rem 0 0;font-size:0.9rem;">{{ L.t('hub_materials_being_prepared') }}</p>
          </div>
        }

        <!-- Related published articles -->
        @if (related().length) {
          <h5 style="font-weight:800;color:var(--text-primary, #2C3E50);margin:2.4rem 0 0.9rem;">{{ L.t('hub_related_guides') }}</h5>
          <div class="row g-3">
            @for (r of related(); track r.id) {
              <div class="col-md-6">
                <a [routerLink]="['/education', r.id]" style="display:block;text-decoration:none;border:1px solid rgba(0,0,0,0.08);border-radius:12px;background:var(--card-bg, #fff);padding:0.9rem 1rem;">
                  ...
                  <div style="font-size:0.9rem;font-weight:700;color:var(--text-primary, #2C3E50);margin-top:2px;">{{ relatedTitle(r) }}</div>
                </a>
              </div>
            }
          </div>
