-- F37 hardening for environments without the local admin@example.com seed. V146
-- repairs the drift when that actor exists; this idempotent fallback only posts
-- rows if drift still remains.

with actor as (
    select coalesce(
        (select id from public.users where email = 'admin@example.com' order by id limit 1),
...
select p.resource_id,
       ...
       format('One-time F37 reconciliation fallback: ledger %s, journal %s before repair.',
